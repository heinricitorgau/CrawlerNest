...
class ARWUAdapter(BaseSourceAdapter):
    """Adapter for normalized ARWU ranking rows."""
    ...
    def _extract_score(self, row: dict[str, Any]) -> float | None:
        direct = row.get("score") or row.get("total_score")
        if direct is not None:
            return self._safe_float(direct)
        scores = row.get("scores")
        if isinstance(scores, dict):
            return self._safe_float(scores.get("overall") or scores.get("total") or scores.get("score"))
        return None
