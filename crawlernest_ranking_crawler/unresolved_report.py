...
def get_unresolved_universities(
    *,
    pg_host: str,
    pg_port: int,
    pg_database: str,
            ...
            if _has_entity_resolution_events_table(cur):
                cur.execute(
                    ...
                    event_stats AS (
                        ...
                        WHERE source_name IN ('QS', 'THE', 'ARWU', 'MULTI_SOURCE')
...
def unresolved_rows_to_dicts(rows: list[UnresolvedUniversityRow]) -> list[dict[str, Any]]:
    return [asdict(row) for row in rows]


def write_unresolved_report(rows: list[UnresolvedUniversityRow], output_file: Path) -> None:
