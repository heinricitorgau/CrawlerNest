...
# Caveats that must be stated aloud during the demo.
...
# Claims that must not be made during the demo.
WHAT_NOT_TO_CLAIM = [
    ...
    "Production-scale global deployment (localhost demo, RC-1 scope)",
    "Predictive rank forecasting (no historical depth)",
    "Confidence scores generated by AI (formula-derived only)",
...
def build(reports_dir: Path, snapshot_file: Path, releases_dir: Path) -> str:
    ...
    lines += [
        "## Operational Posture Summary",
        "",
        "These are the RC-1 stable degraded conditions. They are expected, documented,",
        "and caveat-covered. They do not block the demo.",
        "",
        "- QS data is stale (~354 hours since RC-1 packaging) — disclosed in Data Caveats",
        "- THE data not available at RC-1 — shown as Unavailable on /analytics",
        "- ARWU data not available at RC-1 — shown as Unavailable on /analytics",
        "- Subject ranking coverage incomplete at RC-1 — not on primary demo route",
        "- Confidence is Mostly Low (single source, formula-derived) — correct, not inflated",
