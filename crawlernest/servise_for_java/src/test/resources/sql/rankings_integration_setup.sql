...
-- fixtures deliberately leave it empty (these tests assert against
-- analytics.aggregated_rankings and warehouse.ranking_decision_preview instead), but the
-- table must exist or the context fails on a database the pipeline has not built.
