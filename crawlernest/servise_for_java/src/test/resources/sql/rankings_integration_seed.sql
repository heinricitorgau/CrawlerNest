...
-- ETH Zurich (990001) carries every structured column so the recommendation
-- adapter has something to prove it reads them. The rest deliberately leave the
-- newer columns NULL, which is the ordinary case for a crawled admission page
-- and the case a null-handling regression would break.
