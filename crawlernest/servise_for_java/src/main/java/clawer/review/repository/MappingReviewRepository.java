...
public class MappingReviewRepository {
     ...
     *
     * <p>Reads warehouse.v_entity_mapping rather than
     * warehouse.source_university_mapping. That view unions the ranking
     * mappings with warehouse.source_mapping, where non-ranking sources land,
     * under one source_code vocabulary. A university admission page is not a
     * ranking and has no ranking_source_id, but it names universities and so
     * needs the same review as any other source.
     *
    ...
    private static final String DECIDED_SELECT = """
            ...
            LEFT JOIN warehouse.v_entity_mapping m
                ON m.source_code = r.source_code
