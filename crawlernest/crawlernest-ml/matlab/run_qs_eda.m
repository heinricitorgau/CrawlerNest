...
%   last_crawl_snapshot.json (QS 2026, 1,503 rows). They are hard-coded on
...
referenceCorr = dictionary( ...
    ...
    [0.901460, 0.777220, 0.629995, 0.575715, 0.477510, ...
     0.475958, 0.404741, 0.350648, 0.332866]);
...
referenceGap = dictionary( ...
    ...
    [1.9054, 1.6848, 1.5957, 1.5825, 1.4919, 1.2092, 1.1253, 1.0269, 0.6712]);
...
pcaDelta = max(abs(explained(1:2) - [50.6511; 12.9350]));
targetDelta = max(abs([mean(yLab); std(yLab)] - [41.840333; 18.826756]));
...
xlabel(ax, sprintf("%s (published, ranks 1-600)", targetLabel), Interpreter="none");
...
scatter(ax, score(~labelled, 1), score(~labelled, 2), 9, ...
    ...
    DisplayName=sprintf("rank 601-1503, score withheld (n=%d)", sum(~labelled)));
scatter(ax, score(labelled, 1), score(labelled, 2), 14, ranks(labelled), "filled", ...
    MarkerFaceAlpha=0.85, ...
    DisplayName=sprintf("rank 1-600, score published (n=%d)", sum(labelled)));
