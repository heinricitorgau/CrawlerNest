...
Background on the data (measured on the 2026 snapshot, 1,503 universities):
...
- All nine component indicators are present on every record, but a few carry the
  ...
  goes unnoticed.
- ``Overall Score`` is published only for ranks 1-600. The remaining 903 rows
  ...
  600 labelled rows, predict the 903 unlabelled ones.
...
#: The regression target. Numeric for ranks 1-600, ``"n/a"`` beyond that.
