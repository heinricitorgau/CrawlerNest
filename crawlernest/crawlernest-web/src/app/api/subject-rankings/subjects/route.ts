...
export async function GET(request: NextRequest) {
  ...
  for (const apiBaseUrl of apiBaseUrls) {
    const upstreamUrl = new URL(
      "/api/v1/subject-rankings/subjects",
      apiBaseUrl
    );
    ...
    try {
      ...
      lastError = error;
      console.warn(
        "[api/subject-rankings/subjects] upstream_fetch_failed via=%s error=%o",
        apiBaseUrl,
        error
      );
    }
  ...
  return NextResponse.json(
    {
      success: false,
      error:
        "Unable to load subject list. Please confirm the API server is running.",
      debug:
