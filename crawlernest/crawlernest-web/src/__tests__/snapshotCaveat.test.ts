...
describe("snapshot caveat template", () => {
  it("renders the 2026 sentence the constant used to hold", () => {
    expect(snapshotCaveat([2026])).toBe(SNAPSHOT_CAVEAT_2026);
  });

  it("renders today's constant from the held editions", () => {
    expect(CAVEAT_QS_STALE).toBe(snapshotCaveat(DATASET_YEARS));
