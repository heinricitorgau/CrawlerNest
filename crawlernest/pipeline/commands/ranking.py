...
Validation, ingestion, warehouse mapping preview, and aggregation and decision
previews for ranking records. Extracted from run_pipeline.py, where these lived
alongside every other command group.

The landing-and-resolve chain that used to live here (write-ranking-warehouse-preview,
resolve-ranking-entities, unresolved-ranking-entities, refresh-ranking-resolution)
wrote to and re-resolved warehouse.ranking_records_preview. MultiSourceRankingPipeline
is now the sole writer of warehouse.ranking_record and resolves entities as it
ingests, so that chain was removed rather than repointed: a second writer is
exactly what the convergence was meant to eliminate.
...
def _aggregate_ranking_preview(
    *,
    ...
    # The postgres source read the ranking landing table, which is gone along
    # with the command that wrote it.
