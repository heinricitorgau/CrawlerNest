...
try:
    ...
    from pipeline.utils.schema import (
        ensure_postgres_schema,
        ensure_subject_ranking_postgres_schema,
        _is_postgres_duplicate_error,
        _split_sql_statements,
    )
    # _build_dispatch_dependencies and _handle_run_command still call these three.
    from pipeline.commands.canonical import (
        COMMANDS as _CANONICAL_COMMANDS,
        ingest_rankings_payload,
        load_json_payload,
        run_the_rankings_ingestion,
    )
except ModuleNotFoundError:  # pragma: no cover - package import compatibility
    ...
    from .pipeline.utils.schema import (  # noqa: F401
        ensure_postgres_schema,
        ensure_subject_ranking_postgres_schema,
        _is_postgres_duplicate_error,
        _split_sql_statements,
    )
    from .pipeline.commands.canonical import (  # noqa: F401
        COMMANDS as _CANONICAL_COMMANDS,
        ingest_rankings_payload,
        load_json_payload,
        run_the_rankings_ingestion,
...
def _rebuild_preview_and_resolve(
    *,
    ...
    """Land the admission preview artifact, then refresh admission resolution.

    This ran four steps until the ranking landing chain was deleted: the two
    removed here wrote warehouse.ranking_records_preview and resolved entities
    against it, and both tables are gone (see
    docs/migrations/RANKING_SCHEMA_CONVERGENCE.md). The admission half is the
    live pipeline and is unchanged; the step numbering below is its own.
    """
    try:
