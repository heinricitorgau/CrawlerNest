"""
Country codes, aliases, and country-related utilities.
"""
...
# Comprehensive static fallback for ISO 3166 (over 200 countries)
# This ensures functionality on Windows or systems without /usr/share/zoneinfo/iso3166.tab
ISO_3166_FALLBACK: Dict[str, str] = {
...
# Cache for all available countries
_ALL_COUNTRY_CODES_CACHE: Optional[Dict[str, str]] = None
...
def get_available_countries() -> Dict[str, str]:
    ...
    """
    global _ALL_COUNTRY_CODES_CACHE
    if _ALL_COUNTRY_CODES_CACHE is not None:
        return _ALL_COUNTRY_CODES_CACHE

    countries: Dict[str, str] = {k.lower(): v for k, v in ISO_3166_FALLBACK.items()}
    # Overlay with manual overrides from COUNTRY_CODES
            ...
            with open(path, "r", encoding="utf-8", errors="ignore") as f:
                for raw in f:
                    ...
                    if len(code) == 2 and name:
                        countries[code] = name
            # If we successfully read from a system file, we can stop
            break
        except Exception:
            continue

    _ALL_COUNTRY_CODES_CACHE = countries
    return countries
